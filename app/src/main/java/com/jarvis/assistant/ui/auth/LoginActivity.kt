...
class LoginActivity : AppCompatActivity() {
    ...
    private val googleSignInLauncher = registerForActivityResult(
        ...
        if (account != null) {
            ...
            Log.w("LoginActivity", "Google Sign In returned API error code $apiErrorCode. Attempting device account fallback.")
            tryDeviceAccountFallback()
        } else {
    ...
    private fun setupGoogleSignInClient() {
        val webClientId = try {
            ...
            if (resId != 0) getString(resId).takeIf { it.isNotBlank() } else null
        ...
        if (!webClientId.isNullOrBlank()) {
            Log.d("LoginActivity", "Using configured webClientId: $webClientId")
            gsoBuilder.requestIdToken(webClientId)
        } else {
            Log.d("LoginActivity", "Using standard Google Sign-In profile builder")
        }
    ...
    private fun tryDeviceAccountFallback() {
        try {
            ...
            if (accounts.isNotEmpty()) {
                ...
                Log.i("LoginActivity", "Fallback: using device Google account $email")
                Toast.makeText(this, "Continuing as $email", Toast.LENGTH_SHORT).show()
                onAuthSuccess(name, email, "", null)
                return
            }
        } catch (e: Exception) {
            Log.w("LoginActivity", "Device account fallback failed: ${e.message}")
        }

        signInProgressBar.visibility = View.GONE
        googleSignInBtn.isEnabled = termsCheckBox.isChecked
        Toast.makeText(this, "Sign-in was cancelled or encountered an issue. Please tap again.", Toast.LENGTH_SHORT).show()
    }
